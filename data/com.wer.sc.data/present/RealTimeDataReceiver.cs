...
namespace com.wer.sc.data.present
{
    ...
    public class RealTimeDataReceiver
    {
        ...
        private void Init(string todayDataPath, IPlugin_Market plugin_Market, IPlugin_HistoryData plugin_HistoryData, int lastOpenDate, string[] receiveCodes, List<KLinePeriod> klinePeriods)
        {
            ...
            //初始化历史数据
            this.dataReaderFactory = new DataReaderFactory(plugin_HistoryData.GetDataPath());
            if (lastOpenDate < 0)
                this.lastOpenDate = this.dataReaderFactory.OpenDateReader.LastOpenDate;
            else
        ...
        private void onReceiverPrepared(object sender, DataReceiver_Tick dataReceiver_Tick)
        {
            ...
            {
                dic_Code_Receive.Add(code, new RealTimeDataReceiver_Code(code, date, dataReaderFactory, lastOpenDate, klinePeriods, plugin_Market.MarketData.GetMarketOpenTime(code, date)));
            }
        }

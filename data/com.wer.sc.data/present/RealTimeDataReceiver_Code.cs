...
using com.wer.sc.data.present.data;
...
namespace com.wer.sc.data.present
{
    ...
    public class RealTimeDataReceiver_Code : IRealTimeDataReader
    {
        ...
        public RealTimeDataReceiver_Code(string code, int date, DataReaderFactory dataReaderFactory, int lastOpenDate, List<KLinePeriod> periods, List<double[]> openTime)
        {
            this.code = code;
            this.date = date;
            ...
            for (int i = 0; i < periods.Count; i++)
            {
                ...
                dicKLineData.Add(period, new KLineData_Present(recentKLineDataLoader.GetRecentKLineData(code, lastOpenDate, period), OpenTimePeriodUtils.GetKLineTimeList(date, lastOpenDate, openTime, period), period));
            }
        }        

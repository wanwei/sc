...
namespace com.wer.sc.data.opentime
{
    public class DaySplitter
    {
        ...
        /// <returns></returns>
        public static List<SplitterResult> Split(TimeGetter timeGetter, IDayStartTimeReader dayStartTimeCache)
        {
            List<SplitterResult> indeies = new List<SplitterResult>(500);
            double time = timeGetter.GetTime(0);
            int date = dayStartTimeCache.GetOpenDate(time);
            indeies.Add(new SplitterResult(date, 0));
            for (int index = 1; index < timeGetter.Count; index++)
            {
                time = timeGetter.GetTime(index);
                if (dayStartTimeCache.IsStartTime(time))
                {
                    date = dayStartTimeCache.GetOpenDate(time);
        ...
        public static List<SplitterResult> Split(IKLineData data, IDayStartTimeReader dayStartTimeCache)
        {
            return Split(new KLineDataTimeGetter(data), dayStartTimeCache);
        }
    }
